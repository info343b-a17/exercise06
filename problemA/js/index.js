...
//includes the name of the track.
//
...
//calling the `renderTrack()` function you just defined. Be sure to "clear" the
//previously displayed results first!
//
...
//that your example will work on Internet Explorer. Be sure do this _before_
//your script!
...
//replacing `{searchTerm}` with the passed in search term.
//Send the AJAX request, _then_ encode the response as JSON once it is received, 
//and _then_ should call the `renderSearchResults() function and pass it the 
//encoded data.
//
//IMPORTANT: Your `fetchTrackList()` method should also _return_ the Promise
...
//your `fetchTrackList()` function is called with the user-entered `#searchQuery`
//value. Use the `preventDefault()` function to keep the form from being 
//submitted as usual (and navigating to a different page).
...
//As a special bonus: add the ability to "play" each track listing by clicking
